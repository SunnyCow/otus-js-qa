...
describe('Bookstore API - Find book, add book tests', () => {
  ...
  test.each(isbnsArray)('should add books %s', async (isbns: string|string[]) => {
  ...
  test('should replace book', async () => {
    ...
    const userBooks = userData.data.books.map((b: Book) => b.isbn);
...
describe('Book removal tests', () => {
  ...
  test('should delete one book from user collection', async () => {
    ...
    expect(userData.data.books.map((b: Book) => b.isbn)).toContain(isbn);
    ...
    expect(userData.data.books.map((b: Book) => b.isbn)).not.toContain(isbn);
  });
  ...
  test('should delete all books from user collection', async () => {
    ...
    expect(userData.data.books.map((b: Book) => b.isbn)).toEqual(expect.arrayContaining(isbns));
